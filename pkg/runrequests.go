...
func doRequest(url string) (http.Response, error) {
	...
	if err != nil {
		log.Printf("Failure to create the request: %v", err)
	...
	if ctx_err != nil {
		...
		case <-ctx.Done():
			err := ctx.Err()
			log.Printf("Timeout reached: %v", err)
			return *res, err
		}
	}
...
func do(url string, requests, concurrency int) ([]int, time.Duration, error) {
	...
	for i := 0; i < requests; i++ {
		...
		go func(i int) {
			...
			if err != nil {
				log.Printf("Requests #%d failed: %v", i, err)
				return
			}
...
type Report struct {
	TimeSpent          string         `json:"elapsed_time"`
	RequestsMade       int            `json:"executed"`
	SuccessfulRequests int            `json:"sucessfull"`
	FailedRequests     map[string]int `json:"failures"`
}
